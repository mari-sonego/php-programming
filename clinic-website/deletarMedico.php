...
if(isMetodo("POST")){
    ...
    if($flag){
        $message = "Esse(a) médico(a) tem uma consulta marcado!";
        echo "<script type='text/javascript'>alert('$message');</script>";
    }else{
        $res = Medico :: deletar($_POST["id"]);

        if(!$res){
            $message = "Médico(a) deletado(a) com sucesso";
            echo "<script type='text/javascript'>alert('$message');</script>";
        }else{
            echo"<p>Erro ao deletar o(a) médico(a)</p>";
        }
    }
}
...
if(parametrosValidos($_GET, ["id"])){
    if(Medico:: existe($_GET["id"])){
        ...
        $message = "Esse(a) médico(a) não existe!";
        echo "<script type='text/javascript'>alert('$message');</script>";
        die;
    }
