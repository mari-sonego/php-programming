...
if(isMetodo("POST")){
        ...
        $id = $_POST["id"];
        $flag = false;
        for($a = 0; $a < count($listamedico); $a++ ){
            $con = $listamedico[$a]["id_consutorio"];
            if($con == $id){
                $flag = true;
            }
        }
        ...
        if($flag){
            $message = "Delete os médicos do colsultório";
            echo "<script type='text/javascript'>alert('$message');</script>";
        }else{
            $res = Consultorio :: deletar($_POST["id"]);

            if(!$res){
                $message = "Consultorio deletado com sucesso";
                echo "<script type='text/javascript'>alert('$message');</script>";
            }else{
                echo"<p>Erro ao deletar o consultorio</p>";
            }
        }
}
...
if(isMetodo("GET")){ 
    if(parametrosValidos($_GET, ["id"])){
        if(Consultorio:: existe($_GET["id"])){
            ...
            $message = "Esse consultorio não existe!";
            echo "<script type='text/javascript'>alert('$message');</script>";
            die;
        }
